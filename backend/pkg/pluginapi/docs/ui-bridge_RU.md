...
| `plugin.status` | нет | `result` (`Health`: `{healthy, message, status_json}`) |
| `ui.resize` | `height` | ответа нет |
| `ui.notify` | `level`, `message` | ответа нет |

В v1 `config.test` проверяет уже сохранённую конфигурацию и требует повторной проверки, поскольку может иметь side effect. Если UI хочет проверить текущие поля формы, он сначала должен вызвать `config.save`.

`plugin.status` — read-only runtime status channel: он не имеет side effect, не требует повторной проверки и не показывает уведомление host-а, поэтому предназначен для polling панелью состояния. Канал вызывает `Health`, а `result.status_json` содержит непрозрачный JSON snapshot, определяемый plugin-ом. Plugin-ы с панелью состояния должны использовать его, а не превращать `config.test` в polling.
