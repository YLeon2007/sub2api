...
func TestSelectReleaseAssetsRequiresExactVersionedArchiveAndChecksum(t *testing.T) {
	version := "0.2.7-ru.1"
	exactArchive := "sub2api_0.2.7-ru.1_linux_amd64.tar.gz"
	assets := []Asset{
		{Name: exactArchive + ".evil", DownloadURL: "https://github.com/YLeon2007/sub2api/releases/download/v0.2.7-ru.1/confusable"},
		{Name: exactArchive, DownloadURL: "https://github.com/YLeon2007/sub2api/releases/download/v0.2.7-ru.1/" + exactArchive},
		{Name: "checksums.txt", DownloadURL: "https://github.com/YLeon2007/sub2api/releases/download/v0.2.7-ru.1/checksums.txt"},
	}
...
func TestExpectedChecksumForFileRequiresOneExactBasename(t *testing.T) {
	name := "sub2api_0.2.7-ru.1_linux_amd64.tar.gz"
...
func TestUpdateMutationsUseOperationLock(t *testing.T) {
	...
	go func() {
		close(started)
		_ = svc.applyReleaseAssets(context.Background(), "0.2.7-ru.1", nil)
		close(done)
	}()
	<-started
	select {
