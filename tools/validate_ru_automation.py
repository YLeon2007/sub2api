...
APPROVED_DEPLOY_COMPOSE_SHA256 = "04d614c4af671dcd44a82c5f646346925dd503e7d0dfd563cce1be927dfa44f9"
...
def active_shell_script_invocation_count(text: str, script: str) -> int:
    active_text = "\n".join(
        line for line in text.splitlines() if not line.lstrip().startswith("#")
    )
    logical_text = re.sub(r"\\\s*\n\s*", " ", active_text)
    ...
    for line in logical_text.splitlines():
        ...
        if tokens[:2] == ["-", "run:"]:
            ...
            tokens = tokens[1:]
        if tokens == ["bash", script] or tokens == ["/bin/bash", script]:
            count += 1
        elif tokens == ["sh", script] or tokens == ["/bin/sh", script]:
            count += 1
    return count
...
def validate_ci(text: str, errors: list[str]) -> None:
    ...
    for installer_test in (
        ...
        "deploy/tests/sync-runtime-from-image-test.sh",
        "deploy/tests/docker-runtime-resources-test.sh",
    ):
        require(
            active_shell_script_invocation_count(text, installer_test) == 1,
...
def validate_goreleaser_config(path: Path, errors: list[str]) -> None:
    ...
    require("draft: true" in text, f"{path}: GitHub Release must be created as a draft", errors)
    require("prerelease: false" in text, f"{path}: RU Release must be stable, not a prerelease", errors)
    require(
        re.search(
            r"(?m)^\s+tags:\s*$\n\s+-\s+['\"]?\{\{ \.Env\.RELEASE_VERSION \}\}['\"]?\s*$",
            text,
        )
        is not None,
        f"{path}: GHCR must publish only the exact RELEASE_VERSION tag",
        errors,
    )
    forbidden = [
        "DOCKERHUB",
...
def validate_repo(
    ...
    tracked_goreleaser_config = repo_root / ".goreleaser.yaml"
    if tracked_goreleaser_config.exists():
        validate_goreleaser_config(tracked_goreleaser_config, errors)
    if goreleaser_config and goreleaser_config.resolve() != tracked_goreleaser_config.resolve():
...
release:
  ...
  prerelease: false
"""
    with tempfile.TemporaryDirectory(prefix="ru-goreleaser-guard-") as directory:
        config_path = Path(directory) / "goreleaser.yaml"
        config_path.write_text(valid_goreleaser, encoding="utf-8")
        valid_goreleaser_errors: list[str] = []
        validate_goreleaser_config(config_path, valid_goreleaser_errors)
        assert not valid_goreleaser_errors, valid_goreleaser_errors

        goreleaser_mutations = {
            "draft": valid_goreleaser.replace("draft: true", "draft: false", 1),
            "prerelease": valid_goreleaser.replace("prerelease: false", "prerelease: true", 1),
            "version-tag": valid_goreleaser.replace(
                "      - '{{ .Env.RELEASE_VERSION }}'",
                "      - latest",
                1,
            ),
            "before-hook": valid_goreleaser + "\nbefore:\n  hooks:\n    - go mod tidy -C backend\n",
        }
        for mutation, payload in goreleaser_mutations.items():
            config_path.write_text(payload, encoding="utf-8")
            mutation_errors: list[str] = []
            validate_goreleaser_config(config_path, mutation_errors)
            assert mutation_errors, f"unsafe GoReleaser mutation was accepted: {mutation}"
    ...
    safe_ru_payment = "https://github.com/YLeon2007/sub2api/blob/v0.2.7-ru.1/docs/PAYMENT_RU.md"
    validate_updater_installer_integrity_texts(
...
jobs:
  automation-guards:
    steps:
      ...
      - run: bash deploy/tests/sync-runtime-from-image-test.sh
      - run: /bin/sh deploy/tests/docker-runtime-resources-test.sh
      - run: |
...
""",
        ...
        valid_ci_fixture = (workflow_dir / "backend-ci.yml").read_text(encoding="utf-8")
        valid_release_fixture = (workflow_dir / "release.yml").read_text(encoding="utf-8")
        for workflow_name, fixture, validator, invocation, script in (
            (
                "backend-ci.yml",
                valid_ci_fixture,
                validate_ci,
            ...
            commented_fixture = fixture.replace(invocation, f"      # {invocation.strip()}", 1)
            commented_errors: list[str] = []
            validator(commented_fixture, commented_errors)
            assert any(
                "must run exactly one" in error and script in error
                for error in commented_errors
            ), f"{workflow_name}: commented-out {script} unexpectedly satisfied the policy"
