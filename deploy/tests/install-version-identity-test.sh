...
#!/bin/sh
printf '%s\n' '2026-09-05T20:00:00+03:00	INFO	stdlog	Sub2API 0.2.3-ru.1 (commit: 576932739818cae8249090970059d59efa99fb7b, built: 2026-09-05T20:00:00+03:00)	{"service":"sub2api","env":"bootstrap","legacy_stdlog":true}'
EOF
...
ROOT_DIR="$ROOT_DIR" CASE_INSTALL_DIR="$INSTALL_DIR" bash -c '
    ...
    if [ "$current" != "0.2.3-ru.1" ]; then
        printf "expected full RU version 0.2.3-ru.1, got %s\n" "$current" >&2
        exit 1
    fi
    ...
    for valid in 0.2.3-ru.1 v0.2.3-ru.1; do
        normalized=$(validate_version "$valid")
        if [ "$normalized" != "v0.2.3-ru.1" ]; then
            printf "expected normalized v0.2.3-ru.1, got %s\n" "$normalized" >&2
            exit 1
        fi
    done

    for invalid in v0.2.1 v0.2.1-ru.0 v0.2.1-ru.01 v0.2.3-ru.1-debug; do
