...
All fork deployment defaults use the immutable image `ghcr.io/yleon2007/sub2api:0.2.5-ru.1`. Future releases must update the image tag deliberately; do not switch production to a mutable third-party `latest` tag.
...
curl -fsSLo "$tmpdir/docker-deploy.sh" \
  https://raw.githubusercontent.com/YLeon2007/sub2api/v0.2.5-ru.1/deploy/docker-deploy.sh
...
Replace `<NEW_RU_VERSION>` only with a published immutable tag from [fork Releases](https://github.com/YLeon2007/sub2api/releases), for example `0.2.5-ru.1`.
...
| `GATEWAY_OPENAI_COMPACT_MODEL` | Optional, default `gpt-5.5` | Upstream model for OpenAI `/responses/compact`; use an older supported model only when the compact endpoint requires it |
...
Apple named volumes persist data. An in-container process supervisor lets the Web UI update-and-restart flow relaunch the active binary, but it does not restart stopped containers or start the stack after a host reboot; run `up` again in that case. This path is intended for local use; Docker Compose remains the recommended server deployment. See [APPLE_CONTAINER.md](APPLE_CONTAINER.md).
...
curl -fsSLo "$tmpdir/install.sh" \
  https://raw.githubusercontent.com/YLeon2007/sub2api/v0.2.5-ru.1/deploy/install.sh
