...
Apple `container` 1.1 не предоставляет restart policies (политики перезапуска), автоматический запуск, планирование health checks (проверок работоспособности), Docker API socket или полную Compose-оркестрацию. Скрипт `apple-container.sh` выполняет упорядоченный запуск и проверки готовности при вызове. Внутри контейнера приложения небольшой supervisor (диспетчер процессов) повторно запускает процесс Sub2API после команды перезапуска из Web UI; остановленные контейнеры и весь стек он не запускает.
...
```dotenv
APPLE_CONTAINER_SUB2API_IMAGE=ghcr.io/yleon2007/sub2api:0.2.7-ru.1
...
PostgreSQL volume монтируется в `/var/lib/postgresql`, сохраняя стандартный дочерний data directory PostgreSQL 18. Данные Sub2API и обновляемый runtime binary используют отдельные дочерние каталоги в `sub2api-apple-data`; Redis также хранит данные ниже своей точки монтирования Apple volume. Это требуется потому, что Apple named volumes не поддерживают Docker-поведение copy-up и назначение владельца mount point.
...
Web UI использует тот же flow (сценарий), что и Docker deployment: загружает release с GitHub, атомарно заменяет активный executable и запрашивает перезапуск приложения. В Docker Compose перезапуск обеспечивает restart policy, а в Apple application container — встроенный process supervisor.

Активный executable хранится в `sub2api-apple-data`, поэтому последующие `up`, `restart` или `up --recreate` не отменяют обновление, загруженное из Web UI. Скрипт сохраняет рядом ID настроенного базового образа; если `APPLE_CONTAINER_SUB2API_IMAGE` начинает указывать на другой image ID, новым активным executable становится `/app/sub2api` из этого образа. Поэтому явное обновление image остаётся приоритетным, а in-place update (обновление на месте) сохраняется при обычном пересоздании контейнера приложения.

Если прямой доступ к GitHub отсутствует, задайте `UPDATE_PROXY_URL` с адресом proxy, доступным из VM Apple container. Proxy, слушающий только `127.0.0.1` на Mac, недоступен по этому адресу из VM; используйте надлежащим образом ограниченный listener на host gateway.
...
SUB2API_IMAGE=ghcr.io/yleon2007/sub2api:0.2.7-ru.1 # Должен совпадать с APPLE_CONTAINER_SUB2API_IMAGE в .env.
