...
Текущий русифицированный релиз: `v0.2.7-ru.1`.
...
## Что изменилось в v0.2.7

### Новые возможности

- **Seedance (Ark) native video API** — нативный API видео для Volcano Ark: асинхронные операции создания, запроса статуса и удаления задач без преобразования `content[]` в OpenAI `messages` или Grok `prompt`. Billing выполняется по upstream `usage.completion_tokens`, ответы сохраняют native format. Подробности: [`docs/seedance-api_RU.md`](docs/seedance-api_RU.md).
- **Plugin HostService** — общий host service для локальных plugins. Добавлены namespace-isolated KV storage через Redis и account directory, ограниченный capabilities, объявленными plugin-ом. Старые plugins без `InitHostServices` остаются совместимыми.
- **Read-only plugin status** — добавлены `GET /admin/plugins/:id/status` и UI channel `plugin.status`. Plugin UI может безопасно опрашивать `Health` и получать непрозрачный `status_json` без повторной проверки администратора и без side effects.
...
- Исправлена отправка Antigravity Gemini bare model names вместе с `thinkingConfig`, которая могла приводить к upstream HTTP `404`.
- Исправлены SSE comment heartbeats в native stream Antigravity Gemini: они больше не прерывают stream в клиентах `go-genai` и `python-genai`.
- Для Kimi China Coding Plan ошибка quota exhaustion HTTP `403` больше не приводит к permanent disable account: account ставится на временную паузу до reset time текущего окна.
- Исправлен HTTP `400` при преобразовании Codex built-in tools с root-level union schema (`oneOf` / `anyOf` / `allOf`) из `/v1/responses` в `/v1/messages`.
- Исправлен HTTP `400` для DeepSeek reasoning mode при Chat fallback: теперь сохраняется обязательное поле `reasoning_content`.
- В Antigravity converter точечно удаляется проблемная строка `x-anthropic-billing-header:` из начала top-level `system`, не затрагивая user messages, tools, authentication и native Anthropic path. Подробности: [`docs/ANTIGRAVITY_ATTRIBUTION_429_RU.md`](docs/ANTIGRAVITY_ATTRIBUTION_429_RU.md).

## Что изменилось в v0.2.5

- **OpenCode** стал полноценной платформой: режим `Zen` работает по pay-as-you-go (оплата по факту), а `GO` — по подписке с окнами 5h/weekly/monthly. Adaptive routing (адаптивная маршрутизация) выбирает нативный `Responses`, `Chat Completions` или `Anthropic Messages` по модели; платформа доступна также в Composite Groups, quota monitoring и Channel Monitor.
- **Site Billing Mode** объединяет два прежних переключателя в три операторских режима: `Recharge & Subscription` (по умолчанию), `Recharge only` и `Subscription only`. Режимы согласованно меняют навигацию и purchase page; `Subscription only` запрещает balance top-up orders, но не затрагивает redeem codes, affiliate payouts и другие начисления баланса.
- Для subscriptions добавлены batch assignment (назначение до 100 пользователям) и bulk actions: изменение срока, reset quota, revoke и restore. Для API keys доступно bulk edit лимитов, срока, IP-правил и rate windows; администратор может массово удалять обычных пользователей, но не admin accounts.
- **OpenAI OAuth / Codex Images**: явно поддержанные GPT Image models направляются на нативные `/images/generations` и `/images/edits`, включая streaming и usage accounting; неизвестные/будущие модели не отправляются туда автоматически и сохраняют Responses fallback.
- Для OpenAI Responses WebSocket коэффициенты динамической ёмкости OAuth/API Key pools по умолчанию повышены с `1.0` до `5.0`. Это именно множитель `ceil(account concurrency × factor)`, ограниченный `max_conns_per_account`, а не фиксированные пять соединений; также улучшены пробуждение очереди и очистка закрытых connections.
- Модель для OpenAI `/responses/compact` по умолчанию изменена с `gpt-5.4` на `gpt-5.5`; тот же default указан во всех Compose-вариантах через `GATEWAY_OPENAI_COMPACT_MODEL`.
- Apple `container` теперь поддерживает update-and-restart из Web UI: активный binary хранится в named volume и переживает обычный recreate, встроенный supervisor перезапускает процесс, а смена image ID снова делает binary из образа authoritative (приоритетным). Host-level autostart по-прежнему отсутствует.
- Релиз содержит две разные миграции с номером `238`: `238_opencode_go_platform.sql` добавляет `opencode_go` в ограничения платформ, а `238_purge_unlimited_user_platform_quotas.sql` удаляет записи квот, где все три лимита равны `NULL`. Runner отслеживает полный filename, поэтому применяет обе по порядку; перед обновлением сделайте проверенный PostgreSQL backup.
- Официальный upstream не меняет `backend/go.mod`, `backend/go.sum`, `frontend/package.json` или `frontend/pnpm-lock.yaml`. Русский release дополнительно обновляет `google.golang.org/grpc` с `v1.82.1` до `v1.83.2`, закрывая `GO-2026-6443` (panic из-за отсутствующего `authority`/`Host`) и `GO-2026-6348` (исчерпание heap через фрагментацию HTTP/2 DATA frames); совместимые transitive dependencies (транзитивные зависимости) обновлены `go mod tidy`.
- **Release hardening** (усиление публикации): tracked `.goreleaser.yaml` (отслеживаемая конфигурация) теперь разрешает только immutable GHCR version tag (неизменяемый тег конкретной версии), создаёт GitHub Release как draft (черновик) и исключает Docker Hub/mutable aliases (изменяемые псевдотеги); `sync-runtime-from-image.sh` автоматически восстанавливает прежний binary и перезапускает сервис при сбое atomic replacement (атомарной замены), включая отказ второго `mv`.
...
## Политика Codex Fast/Flex

Настройка `Системные настройки -> Gateway service -> OpenAI Fast/Flex policy` обрабатывает только поле `service_tier` в request body. Она не изменяет model catalog клиента Codex и сама по себе не добавляет в Codex UI пункт Speed или команду `/fast`.

- `pass` сохраняет переданный клиентом `service_tier`; значение `fast` нормализуется в upstream-значение `priority`.
- `filter` удаляет `service_tier` и отправляет запрос с обычным приоритетом.
- `block` отклоняет совпавший Fast/Flex request.
- `force_priority` принудительно задаёт `priority`, что тарифицируется по Priority/Fast цене. Чтобы не менять смысл существующих правил при обновлении, selector `all` совпадает только с явно переданным tier. Для OpenAI request без `service_tier` создайте отдельное правило `service_tier=missing + force_priority`; для non-OpenAI platforms отсутствующий tier не подставляется.

Пункт `/fast` зависит от model catalog самого Codex: в нём должны присутствовать `additional_speed_tiers: ["fast"]` и соответствующие `service_tiers`. При API Key или custom provider настройка `force_priority` может реально отправлять request как Fast, но UI не покажет Speed, если этих полей нет в catalog.

Default tier можно задать в `~/.codex/config.toml`:
...
`service_tier = "fast"` добавляет Fast-настройку в request, а `features.fast_mode` лишь включает клиентскую функцию. Итоговое upstream-значение `priority` проверяйте в usage records Sub2API.
...
umask 077
tmpdir="$(mktemp -d)"
curl -fsSLo "$tmpdir/install.sh" https://raw.githubusercontent.com/YLeon2007/sub2api/v0.2.7-ru.1/deploy/install.sh
...
umask 077
tmpdir="$(mktemp -d)"
curl -fsSLo "$tmpdir/docker-deploy.sh" https://raw.githubusercontent.com/YLeon2007/sub2api/v0.2.7-ru.1/deploy/docker-deploy.sh
...
```text
ghcr.io/yleon2007/sub2api:0.2.7-ru.1
```
...
```bash
docker pull ghcr.io/yleon2007/sub2api:0.2.7-ru.1
