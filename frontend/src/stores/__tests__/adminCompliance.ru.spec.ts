...
function status(required = true) {
  return {
    ...
    document_url_zh: 'https://github.com/YLeon2007/sub2api/blob/v0.2.0-ru.1/docs/legal/admin-compliance.zh.md',
    document_url_en: 'https://github.com/YLeon2007/sub2api/blob/v0.2.0-ru.1/docs/legal/admin-compliance.en.md',
    document_url_ru: 'https://github.com/YLeon2007/sub2api/blob/v0.2.0-ru.1/docs/legal/admin-compliance.ru.md',
...
describe('Russian admin compliance contract', () => {
  ...
  it('selects the Russian phrase and keeps Russian document metadata', () => {
    ...
    expect(store.status?.document_url_ru).toContain('/blob/v0.2.0-ru.1/')
  })
  ...
  it('pins fallback document URLs to the immutable RU release', () => {
    ...
    expect(store.status?.document_url_zh).toContain('/blob/v0.2.0-ru.1/')
    expect(store.status?.document_url_en).toContain('/blob/v0.2.0-ru.1/')
    expect(store.status?.document_url_ru).toContain('/blob/v0.2.0-ru.1/')
  })
